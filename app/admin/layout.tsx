    ...
    <div className="grid min-h-screen grid-cols-1 bg-bone-100 lg:grid-cols-[240px_1fr]">
      <aside className="hidden border-r border-stone-200 bg-bone-50 lg:flex lg:flex-col">
          ...
          {navItems
            ...
            .map((item) => (
              <Link
                key={item.href}
                href={item.href}
                className="flex items-center gap-3 rounded-sm px-3 py-2.5 text-sm text-ink-950 hover:bg-bone-100"
              >
        ...
        <div className="space-y-2 border-t border-stone-200 px-3 py-4">
          <Link
            href="/"
            className="flex items-center gap-3 rounded-sm px-3 py-2.5 text-sm text-stone-600 hover:bg-bone-100"
          >
          ...
          {navItems
            ...
            .map((item) => (
              <Link
                key={item.href}
                href={item.href}
                className="shrink-0 rounded-sm px-3 py-2 text-xs uppercase tracking-[0.08em] text-ink-950 hover:bg-bone-100"
              >
                {item.label}
              </Link>
            ))}
